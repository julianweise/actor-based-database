...
public class ADBJoinWithNodeSession extends ADBLargeMessageActor {
    ...
    @NoArgsConstructor
    @Getter
    public static class ForeignAttributes implements ADBLargeMessageSender.LargeMessage {
    ...
    public Receive<Command> createReceive() {
        return this.createReceiveBuilder()
                   ...
                   .onMessage(ForeignAttributes.class, this::handleForeignAttributes)
                   .onMessage(PartitionsJoinedWrapper.class, this::handlePartitionsJoined)
                   .onMessage(SemiMaterializedTuplesWrapper.class, this::handleTuplesSemiMaterialized)
    ...
    private Behavior<Command> handleAllPartitionsHeader(AllPartitionsHeaderWrapper wrapper) {
        ...
        if (wrapper.response.getHeaders().size() < 1) {
            this.getContext().getLog().warn("No Partition headers present. Concluding session ...");
            this.getContext().getSelf().tell(new CheckConclude());
        }
        return Behaviors.same();
    }

    private Behavior<Command> handleForeignAttributes(ForeignAttributes command) {
        for(Map.Entry<Integer, List<Integer>> mapping : command.getJoinCandidates().entrySet()) {
            ...
            for(int fPartitionId : mapping.getValue()) {
                this.joinPartitions(lPartitionId, fPartitionId, command.joinAttributes);
            }
        }
        return Behaviors.same();
    }
    ...
    private void joinPartitions(int lPartitionId, int fPartitionId,
                                Map<Integer, Map<String, List<ADBPair<Comparable<Object>, Integer>>>> joinAttributes) {
        this.remainingResults.incrementAndGet();
        this.spawnExecutor(lPartitionId, fPartitionId, false, joinAttributes.get(fPartitionId));
        if (ADBSlave.ID == this.remoteNodeId) {
            return;
        }
        this.remainingResults.incrementAndGet();
        this.spawnExecutor(lPartitionId, fPartitionId, true, joinAttributes.get(fPartitionId));
    }
    ...
    private void spawnExecutor(int lPartId, int fPartId, boolean isReversed,
        ...
        val behavior = ADBPartitionJoinExecutorFactory.createDefault(joinQuery, this.localPartitions.get(lPartId),
                foreignAttributes, lPartId, fPartId, respondTo, isReversed);
        this.getContext().spawn(behavior, name);
    }

    private Behavior<Command> handlePartitionsJoined(PartitionsJoinedWrapper wrapper) {
        val respondTo = getContext().messageAdapter(ADBPartition.SemiMaterializedTuples.class,
                SemiMaterializedTuplesWrapper::new);
        ADBPartition.SemiMaterializeTuples message = new ADBPartition.SemiMaterializeTuples(respondTo,
                wrapper.joinResults.getForeignPartitionId(), wrapper.joinResults.getJoinTuples(),
                wrapper.joinResults.isReversed());
        this.localPartitions.get(wrapper.joinResults.getLocalPartitionId()).tell(message);
        return Behaviors.same();
    }

    private Behavior<Command> handleTuplesSemiMaterialized(SemiMaterializedTuplesWrapper wrapper) {
        this.getContext().getLog().debug("Successfully semi-materialized tuples for join with " + this.remoteNodeId);
        this.remainingResults.decrementAndGet();
        List<ADBSemiMaterializedPair> tuples = wrapper.result.getResults();
        val message = new ADBJoinWithNodeSessionHandler.MaterializeAndReturn(wrapper.result.getForeignPartitionId(),
                tuples, wrapper.result.isReversed());
        this.sessionHandler.tell(message);
        this.getContext().getSelf().tell(new CheckConclude());
        return Behaviors.same();
    }
